...
async function sendWithRetry(url, data, retries = 1) {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      // Detectar bloqueo de Brave o problemas de red
      let errorMsg = err.message;
      if (err.message.includes('Failed to fetch') || err.name === 'TypeError') {
        errorMsg = 'El navegador bloqueó la conexión. Si usas Brave, desactiva los escudos (icono del león) para este sitio o prueba con otro navegador.';
      }
